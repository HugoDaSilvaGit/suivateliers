...
<html lang="fr">
	...
	<head>
		<meta charset="utf-8">
		<title>Suivis Ateliers - Listes Ateliers</title>
				...
				<tbody>
					...
					<?php foreach( $ateliers as $unAtelier ){ ?>
						<tr>
                            ...
                            <td><?php 
							$date = date('Y-m-d', strtotime($unAtelier[ 'dateHeureProgramme' ]));
							$heure = date('H:i:s', strtotime($unAtelier[ 'dateHeureProgramme' ]));
							echo 'Le '.$date.' à '.$heure; ?></td>
                            <td><?php echo $unAtelier[ 'duree' ] ; ?></td>
                            <td><?php echo $unAtelier[ 'nbPlace' ] ; ?></td>
                            <td><?php echo $unAtelier[ 'theme' ] ; ?></td>
							<td>
								<a href="vue-liste-stagiaires.php?atelier=<?php echo $unAtelier[ 'numero' ]; ?>" type='button' class='btn btn-primary'> Liste Inscriptions</a>
							</td>
						</tr>
					<?php } ?>
